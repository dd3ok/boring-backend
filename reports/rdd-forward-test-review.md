...
- `skills/rdd-review/SKILL.md`: review order, P0-P4 severity definitions, review-only behavior, and report shape.
- `skills/rdd-review/references/guard-catalog.md`: idempotency, concurrent read-then-write, stock/count invariants, pagination, error mapping, and local-only assumption checks.
- `skills/rdd-review/references/security-guard-catalog.md`: object/function authorization and fail-closed auth checks for state transitions accepting user-controlled IDs.
- `skills/rdd-review/references/performance-guard-catalog.md`: list endpoint bounds, payload size, query path, and resource saturation checks.
- `skills/rdd-review/references/resilience-guard-catalog.md`: transaction/lock safety, retry/idempotency interaction, and local-vs-distributed safety checks.
- `skills/rdd-review/references/operations-guard-catalog.md`: production-readiness evidence, observability, SLO, backup/restore, compatibility, and resource-risk checks.
- `skills/rdd-review/references/forward-test-prompts.md`: expected pass signal for an RDD review of the generated order API.
