...
3. For explicitly requested evidence from a named staging or production environment, including live telemetry, read [production evidence](references/production-evidence.md) first. Follow its staged gate: establish the safety envelope through read-only inspection, prepare a bounded plan, obtain separate execution approval, then perform invasive action.
