...
Read this file only when the task touches latency, throughput, high-traffic endpoints, list/search/export/bulk operations, DB query performance, N+1 risk, indexing, connection/thread pools, payload size, caching for speed, load testing, or performance optimization.
...
| Cache for speed | Define key scope, TTL, invalidation, stampede behavior, and caller/security context. A cache that can return stale or unauthorized data is not a safe optimization. |
