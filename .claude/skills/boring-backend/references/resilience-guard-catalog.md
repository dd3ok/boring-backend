...
Read this file only when the task touches external calls, retries, timeouts, locks, transactions, queues/events, caches, quotas, throttling, backpressure, overload control, or distributed/MSA behavior.
...
| Transaction retry | When using serializable/repeatable-read isolation, pessimistic locks, or optimistic versions, handle serialization failures, deadlocks, and version conflicts with safe retry or explicit conflict responses. |
...
- Can two transactions deadlock, serialize-fail, or overwrite each other without safe handling?
