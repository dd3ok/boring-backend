...
| State transition and dependent side effects | Only the operation that atomically wins the transition may perform dependent side effects. Keep the transition and effects in one transaction when they share a transactional store; otherwise persist a durable work record and reconcile incomplete effects. Test competing valid transitions. |
