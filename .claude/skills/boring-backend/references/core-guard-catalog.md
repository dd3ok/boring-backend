...
| Risk | Implement |
|---|---|
| Idempotency | Use the narrowest durable mechanism the contract supports: natural operation/business ID, unique constraint, atomic conditional write, or durable idempotency key plus request fingerprint. Reject the same identity with a payload/intent mismatch. Release/expire a claim and retry only after proving no side effect executed and, if a transaction began, rollback completed. If any effect may have executed, including an external effect outside a rolled-back transaction, retain a durable pending/unknown state and reconcile it before re-execution; retry an executed effect only when it is independently idempotent under the same identity. Persist/replay contract-final outcomes, bind claim and outcome atomically to the operation, and prove retries or concurrent duplicates cannot duplicate side effects. |
...
- Does payload mismatch reject, contract-final replay stay stable, retry require proof that no effect executed, and any executed or uncertain effect reconcile durably before re-execution?
